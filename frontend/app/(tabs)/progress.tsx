...
import { View, Text, StyleSheet, ScrollView, ActivityIndicator, TouchableOpacity, Alert, Platform, TextInput } from "react-native";
...
import { Card, SectionTitle, WeekBars, LineChart1RM } from "@/src/components/UI";
...
type WeightPoint = { id?: string; date: string; weight: number; time: number };
...
export default function Progress() {
  ...
  const load = useCallback(async () => {
    try {
      const [w, p, wk, hist, mv, pr, weights] = await Promise.all([
  ...
  return (
    <ScreenBackground bg="progress">
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <BodyCompositionCard profile={profile} weightLogs={weightLogs} onSaved={load} />
...
function BodyCompositionCard({
  ...
  weightLogs.forEach((item) => {
    weightsMap.set(item.date, { id: item.id, date: item.date, weight: Number(item.weight_kg), time: parseLocalISO(item.date).getTime() });
  });
        ...
        {weights.length > 0 ? (
          <View style={styles.weightHistoryList} testID="weight-history-list">
            ...
            {weights.slice().reverse().slice(0, 5).map((point) => (
              <View key={point.id || point.date} style={styles.weightHistoryRow}>
                <View style={{ flex: 1 }}>
